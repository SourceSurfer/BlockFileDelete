...
//
	...
	#pragma alloc_text(PAGE, ScannerPreCreate1)
...

{
		...
		if (Data->Iopb->MajorFunction == IRP_MJ_WRITE || Data->Iopb->MajorFunction == IRP_MJ_SET_INFORMATION || Data->Iopb->MajorFunction == IRP_MJ_CREATE) {
			...
			if (RtlCompareUnicodeString(&Data->Iopb->TargetFileObject->FileName, &ourFile, FALSE) == 0) {
				...
				Data->IoStatus.Status = FILE_DELETE_ON_CLOSE;
    ...
    if (NT_SUCCESS( status )) {
        ...
        if (context->RescanRequired) {
            ...
            if (!safe) {
               ...
               // DbgPrint( "!!! scanner.sys -- нецензурная лексика, обнаруженная в precleanup !!!\n" );
            }
        }
...
--*/
{
	...
	DbgPrint("", RequesterContext, OperationStatus, ParameterSnapshot->MajorFunction, FltObjects->Filter);
